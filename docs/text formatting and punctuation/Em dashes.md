...
As with the marks it replaces, it always appears mid-sentence where you would normally find another mark of punctuation.
...
1. If the sentence contains multiple phrases or clauses, try using an em dash to reveal its structure at a glance.
2. If you would add a longer pause (for comprehension or for dramatic effect) while saying the sentence, then the longer pause should be indicated by an em dash.
3. If minus signs are in close proximity (~in the same grouping of sentences), then do not use an em dash. In this case, use the comma or separate your clauses into consecutive sentences.
